...
from sqlalchemy import create_engine, ForeignKey, Column, Sequence
from sqlalchemy.ext.declarative import declarative_base  
from sqlalchemy.orm import sessionmaker, relationship
from sqlalchemy.engine.url import URL
from sqlalchemy.dialects.postgresql import ARRAY, BIGINT,BOOLEAN, \
     BYTEA, DOUBLE_PRECISION, INTEGER, JSONB, TEXT
...
def initialize_db():
   ...
   def push_iteration_db(COV, partition_list, attributes, iteration_number, elapsed_iter_time):
      iteration = Iteration(exp_id = experiment_id,
                            cov = COV.to_json(compression='infer'),
                            kg_mu =  [attributes['KG_mu'][x] for x in attributes['KG_mu'].keys()],
                            kg_lambda = [attributes['KG_lambda'][x] for x in attributes['KG_lambda'].keys()],
   ...
   def push_solution_db(solution):
      solution = Output(exp_id = experiment_id,
   ...
   return push_experiment_db, push_iteration_db, update_exp_time, push_solution_db, close_db
