...
def update_mu_S(mu_n, S_n, lambda_, x, y_n1):
   ...
   mu_n1 = mu_n.reshape(M,1) + ((y_n1-mu_n[x])/(lambda_[x]+S_n[x][x])*np.dot(S_n, e_x))

   #S_n1 = S_n - (1/(lambda_[x]*S_n[x][x]))*S_n.dot(e_x).dot(e_x.transpose()).dot(S_n)
   S_n = S_n - (1/(lambda_[x]+S_n[x][x]))*S_n.dot(e_x).dot(e_x.transpose()).dot(S_n)
...
def KG_multi(mu, S, lambda_, pool):
   ...
   if M >= 10:
      L = pool.map(partial(KG_iteration, mu, S, lambda_,), range(1,M))
