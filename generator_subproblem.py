...
from generator_penalty import generate_penalty_vals_linear
...
def optimize_subproblem(project_network,
    ...
    penalty_plus_constr = {}
    for i in range(1, int(penalty_steps)+1):
        penalty_minus_constr[i] = model.addConstr(s_f >= t[i] - BigM*(1-z[i]), name="p-" + str(i))
        penalty_plus_constr[i]  = model.addConstr(s_f <= t[i] + BigM*z[i], name="p+" + str(i))
    ...
    scenariodetailslog = {}
    scenariodetailslog["Penalty"] = sum([Lambda[i] * model.getVarByName("z"+str(i)).X for i in range(1, int(penalty_steps)+1)])
    scenariodetailslog["Objective func Value"] = model.getAttr('ObjVal')
    scenariodetailslog["Regular Costs"] = scenariodetailslog["Objective func Value"] - scenariodetailslog["Penalty"]
    scenariodetailslog["Project Duration"] = model.getVarByName("s" + str(no_of_nodes-1)).X
    scenariodetailslog["Schedule"] = [model.getVarByName("s"+str(i)).X for i in range(no_of_nodes)]
    scenariodetailslog["Penalty Variables"] = [model.getVarByName("z"+str(i)).X for i in range(1, int(penalty_steps)+1)]
