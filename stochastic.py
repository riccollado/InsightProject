...
def estimate_bounds(attributes, subproblem, scenarios):
   ...
   subproblem['total_scenarios_done'] = subproblem.get('total_scenarios_done', 0) + len(scenarios)
...
def bootstrap_E_STD(subproblem, attributes):
   ...
   boost_mean_dist = bs.bootstrap(values=subproblem_sample_sol, stat_func=bs_stats.mean,
                     alpha=alpha, num_iterations=b, iteration_batch_size=None,
                     is_pivotal=True, num_threads=1, return_distribution=True)   
   
   # Use bootstrap to approximate std
   boost_std_dist = bs.bootstrap(values=subproblem_sample_sol, stat_func=bs_stats.std,
                      alpha=alpha, num_iterations=b, iteration_batch_size=None,
                      is_pivotal=True, num_threads=1, return_distribution=True)   
...
def bootstrap_KG(subproblem, attributes, sample_solutions):
   ...
   # Use bootstrap to approximate std
   boost_std_dist = bs.bootstrap(values=KG_sample_sol, stat_func=bs_stats.std,
                      alpha=alpha, num_iterations=b, iteration_batch_size=None,
                      is_pivotal=True, num_threads=1, return_distribution=True)   
...
def assign_scenarios(partition_list, total_scenarios, method):
   ...
   if method == "Random":
      ...
      assigned_scenarios = np.array([np.random.randint(0,len(partition_list))] * scenarios_length)
      ...
      assigned_scenarios = np.random.choice(elements, scenarios_length, p=probabilities)
...
def assign_scenarios_KG(partition_list, total_scenarios, attributes):
   ...
   for i, subproblem in enumerate(partition_list):
      if subproblem['name'] == selected_name:
         assigned_scenarios_dict[i] = [total_scenarios[j].tolist() for j in range(len(total_scenarios))]
      else:
...
def assign_scenarios_pareto(partition_list, total_scenarios, beta, method):
   ...
   if method=="Pareto_Inverse" or method=="Pareto_Boltzman":
      ...
      num_fronts, front_list, probabilities = pareto_fronts_probabilities(partition_list, beta, method)
...
def rank_by_distance(partition_list):
   ...
   for subproblem in partition_list:
      if subproblem['Z_E'] == np.inf:
         ...
         distance.append(math.pow((math.pow(subproblem['Z_E'], 2) + math.pow(subproblem['Z_std'] - 1, 2)), 0.5))
...
def pareto_fronts_probabilities(partition_list, beta, probability_method):
   ...
   points = [[ -(abs(subproblem['Z_E'])), -(abs(1 - abs(subproblem['Z_std']))) ] 
             for subproblem in partition_list]   
...
def update_row_col(subproblem1, subproblem2, partition_list, attributes):
   ...
   const_vec = [partition_list[i]['constraints'] for i in range(len(partition_list)-2)]
